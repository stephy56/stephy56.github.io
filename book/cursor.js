...
(function () {
  function injectBookLink() {
    try {
      // --- make the dropdown open reliably on hover (adds a small hover "bridge"
      //     across the 1px gap, and opens on keyboard focus / tap too) ---
      if (!document.getElementById('nav-hover-fix')) {
        var st = document.createElement('style');
        st.id = 'nav-hover-fix';
        st.textContent = '.nav-dropdown{position:relative}' +
